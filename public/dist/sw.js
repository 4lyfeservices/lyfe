importScripts("/dist/bundle.js?v=4"), importScripts("/dist/config.js?v=4");
class CVServiceWorker extends EventEmitter {
  constructor(e = __cv$config) {
    super(),
      e.bare || (e.bare = "/o/"),
      (this.addresses =
      ...
      (this.config = e),
      (this.browser = ClassicUV.Bowser.getParser(
  ...
  async fetch({ request: e }) {
    if (
      !e.url.startsWith(location.origin + (this.config.prefix || "/network/"))
    )
      return fetch(e);
    try {
      const t = new ClassicUV(this.config);
      ...
      if (
        ...
        switch (e.destination) {
          case "script":
          case "worker":
            (c.body = `if (!self.__cv && self.importScripts) importScripts('${__cv$config.bundle}', '${__cv$config.config}', '${__cv$config.handler}');\n`),
  ...
  static ClassicUV = ClassicUV;
}
self.CVServiceWorker = CVServiceWorker;
class ResponseContext {
  constructor(e, t, r) {
    const { headers: n, status: s, statusText: i, body: o } = e.blob
      ? {
          status: t.status,
...
class RequestContext {
  ...
  get url() {
    return this.ClassicUV.meta.url;
  }
  set url(e) {
    this.ClassicUV.meta.url = e;
  }
  get base() {
    return this.ClassicUV.meta.base;
  }
  set base(e) {
    this.ClassicUV.meta.base = e;
  }
}
function isHtml(e, t = "") {
  return (
    "text/html" ===
    (ClassicUV.mime.contentType(t || e.pathname) || "text/html").split(";")[0]
  );
}
class HookEvent {
  #e;
